    ...
    fs_1.default.exists(dirpath, exists => {
        if (exists) {
            ...
            exports.mkdirs(path_1.default.dirname(dirpath), () => {
                fs_1.default.mkdir(dirpath, callback);
            });
        }
    });
};
exports.writeFile = (dirpath, data, callback) => {
    fs_1.default.writeFile(dirpath, data, function (err) {
        if (err) {
            return consola_1.default.error(`文件写入失败: ${dirpath}`);
        }
        else {
            if (callback) {
                return callback();
                ...
                return consola_1.default.success(`文件写入成功: ${dirpath}`);
            }
        }
    });
};
